...
async def doc(bot, update):
    try:
        ...
        await ms.edit("三 𝘗𝘳𝘦𝘱𝘢𝘳𝘪𝘯𝘨 𝘵𝘰 𝘳𝘦𝘤𝘦𝘪𝘷𝘦 𝘙𝘰𝘴𝘩𝘪 𝘧𝘪𝘭𝘦...︻デ═一")
        c_time = time.time() 
        try:
           if type == "document":
...
async def next_page(bot, query):
    ...
    btn.insert(0,
        [ 
        InlineKeyboardButton("  ꜰɪʟᴛᴇʀ ʟᴀɴɢᴜᴀɢᴇ ʕʘ̅͜ʘ̅ʔ", callback_data=f"languages#{key}"),
        ] 
    )
    btn.insert(0,
        [ 
...
async def advantage_spoll_choker(bot, query):
    ...
    if int(user) != 0 and query.from_user.id != int(user):
        return await query.answer("T😁 𝗛𝗲𝘆 𝗙𝗿𝗶𝗲𝗻𝗱,𝗣𝗹𝗲𝗮𝘀𝗲 𝗦𝗲𝗮𝗿𝗰𝗵 𝗬𝗼𝘂𝗿𝘀𝗲𝗹𝗳.", show_alert=True)
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data == "close_data":
        ...
        if chat_type == enums.ChatType.PRIVATE:
            ...
            if grpid is not None:
                ...
                try:
                    ...
                    return await query.answer('♥️ Love @MasterXibot ♥️')
            else:
        ...
        if chat_type == enums.ChatType.PRIVATE:
            ...
            if (st.status == enums.ChatMemberStatus.OWNER) or (str(userid) in ADMINS):
                ...
                await query.answer("That's not for you Buddy!", show_alert=True)
    elif "=" in query.data:
        try:
    ...
    if query.data.startswith("file"):
        ...
        try:
            ...
            reply_markup = InlineKeyboardMarkup(btn)
            await client.send_message(int(user_id), f"😒 Oops! Buddy, Your requested content named `{movie}` is not available right now, we are really trying our best to serve you this content, can you please provide us some more details related to your query `{movie}`, \nSend details to Admin : <a href='https://telegram.me/{ADMIN_USRNM}'>**Send here...**</a>\n\n❤ Thank You for the contribution", reply_markup=reply_markup)
        ...
        try:
            ...
            reply_markup_lzdv = InlineKeyboardMarkup(btn_lzdv)            
            reply_markup = InlineKeyboardMarkup(btn)
            await client.send_message(int(user_id), f"🛋 Hey Buddy, Your requested content named `{movie}` is already available in our database! You can easily get this movie by searching it's correct name in our official group...\nSend details to Admin : \n\n❤ Thank You for the contribution", reply_markup=reply_markup)
        ...
        try:
            ...
            reply_markup_lzdv = InlineKeyboardMarkup(btn_lzdv) 
            reply_markup = InlineKeyboardMarkup(btn)
            await client.send_message(int(user_id), f"✅ Hey Buddy, Your requested content named `{movie}` is now available in our database! You can easily get this movie by searching it's correct name in our official group...\n\n❤ Thank You for the contribution", reply_markup=reply_markup)
            await query.edit_message_text(text=f"- __**User notified successfully sweetie...✅**__\n\n⏳**Status** : Upload done ✅.\n🪪**UserID** : `{user_id}`\n🎞**Content** : `{movie}`\n\n\n🦋", reply_markup=reply_markup_lzdv)
        # Delete callback query message
            await query.answer()
            await query.delete()
        ...
        # Delete callback query message
            ...
            print(e)  # print the error message
            await query.answer(f"☣something went wrong Buddy\n\n{e}", show_alert=True)
            return
...
async def auto_filter(client, msg, spoll=False):
    ...
    btn.insert(0,
        [ 
	    InlineKeyboardButton(text="⚡ʜᴏᴡ to ᴅᴏᴡɴʟᴏᴀᴅ⚡", url='https://t.me/tamcinemasdubt'),
        ] 
    )
